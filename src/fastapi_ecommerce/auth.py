...
async def get_current_user(token: str = Depends(oauth2_scheme),
    ...
    try:
        ...
        raise credentials_exception
    user: UserModel | None = (
        await db.execute(select(UserModel).where(
...
async def get_current_seller(current_user: UserModel = Depends(
        get_current_user)):
    """
...
async def get_current_buyer(current_user: UserModel = Depends(
        get_current_user)):
    """
