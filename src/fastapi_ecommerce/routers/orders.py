...
from fastapi_ecommerce.models.orders import Order as OrderModel, OrderItem as OrderItemModel
from fastapi_ecommerce.models.users import User as UserModel
from fastapi_ecommerce.schemas import Order as OrderSchema, OrderList
...
async def _load_order_with_items(db: AsyncSession, order_id: int
                                 ) -> OrderModel | None:
...
async def list_orders(
    ...
    result = await db.scalars(
        select(OrderModel)
        .options(selectinload(OrderModel.items).selectinload(
